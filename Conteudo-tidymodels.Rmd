...
wf_knn_demo <- workflow() %>% 
  add_recipe(receita_knn_demo) %>% 
  add_model(knn_mod)
...
predicoes_knn <- predict(
  object = fit_knn_demo,
  new_data = dados_novos
)


predicoes_knn_com_dados <- bind_cols(
  dados_novos,
  predicoes_knn
) 
...
ggplot(predicoes_knn_com_dados) +
  geom_tile(
    aes(
      x = i_found_partner__attractive,
      y = my_age,

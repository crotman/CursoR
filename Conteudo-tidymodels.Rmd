...
A sessão de exploração nos ajuda fazer alguns testes de sanidades nos dados e a extrair alguns insights que podem ou não ser usados para construir o processo Feature Engineering que pode ajudar o modelo atingir melhores resultados


## Processo de escolha e avaliação do modelo: antes de tudo separar os dados de teste
...
Um dos passos que podem ser definidos na recipes é a identificaão de atributos que não têm papel preditivo e, portanto, não devem ser usados no treinamento e na predição, mas que queremos manter no nosso tibble para identificação das linhas.
...
Quem está acostumado com a lm, sabe que os fatores são transformados automaticamente em dummies, mas isso não acontece com todo os engines.
...
step_zv() retira os as variáveis com variância zero. Isso va acontecer bastante quando temos levels de fatores infrequentes.
...
Sabendo criar uma receita de pré-processamento e relembrando como criar uma interface genérica para um modelo com parsnip() e como selecionar um engine pra ele, podemos criar um pequeno fluxo de trabalho para realizar esse procesamento, usando a biblioteca workflows()
...
Um tipo de passo interessante que pode ser adicionado à recipe é a adição de novos preditores que representam interações entre os preditores adicionais.
...
Aqui vamos usar uma regressão mesmo ao invés de clasificação para facilitar a interpretação dos coeficientes.
...
É sempre melhor depender de interfaces do que de implementações. A parnsip funciona como uma camada de abstraão que oferece uma interface única que se encarrega de cuidar das diferentes implementações.

As interfaces genéricas da parnsip está prontas para receber os parâmetros mais comuns usados nos engines e estão preparadas para passar ao engine o valor destes parâmetros.
...
- step_corr() retira preditores de forma que 2 a 2 não haja mais nenhum com correlação acima do threshold.
...
Executar o cross validation significa estimar o modelo várias vezes em execuções que são completamente diferentes, o que é perfeito para executar em paralelo.

A função fit_resamples() faz todas as estimações. É possível registrar um backend para fazer a execução de forma paralela
...
collect_metrics(fit_com_interacao_resample) %>% gt()
...
No momento de definir o modelo, atribuimos a resposta de tune() aos parãmetros do modelo que queremos variar para fins de tuning
  ...
  results %>% 
    ...
    geom_text(
      aes(
        x = penalty,
        y = mixture,
        label = number(mean, accuracy = .001),
      ),
      size = 3
    ) +
    scale_x_log10() +

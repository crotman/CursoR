...
Abaixo vemos os três gráficos que ela gera:
...
gg_tsdisplay(crime_estado_ts, y = hom_doloso_diff_log )
...
crime_estado_ts %>% 
  features(hom_doloso, unitroot_kpss )
...
previsao <- forecast(
  ...
  new_data = 
    tsibble(
      ...
      p67_temp = replicate(expr = 27, n = 5),
      index = data
    )
)
...
fora <- previsao_fora_amostra(
  modelo = modelo_puro,
  data = make_date(2018,1,1),
  dado = crime_estado_ts
)
...
modelo_bobo <- function(dado){
  dado %>% 
    model(snaive = SNAIVE(hom_doloso ~ drift() + lag("year")))
}
...
write_rds(performance_decomp, "dados/crime_rio/performance_bobo")

...
Bruno Crotman ([crotman\@gmail.com](mailto:crotman@gmail.com){.email})
...
O livro The Art of Statistics [@spiegelhalter] propõe o seguinte ciclo para o processo de extrair insights de um conjunto de dados. Este processo pode envolver ferramentas estatísticas sofisticadas ou não.
...
[Formato de relatório. Análise mais direcionada e com conclusões do que exploratória](https://crotman.shinyapps.io/airbnb)
...
-   `r a_r("vetores atômicos")` (seus elementos são do mesmo tipo de dados primário)

-   `r a_r("listas")` (seus elementos, que são vetores atômicos, são de tipos de dados primários diferentes)
...
Um conceito importante é o de `r a_r("recycling")`.
...
Um `r a_r("vetor nomeado")` pode ser criado como abaixo

```{r}

vetor_nomeado <- c(primeiro = 1, segundo = 2, terceiro = 3)

vetor_nomeado

```

Além de acessar o elemento do vetor pelo índice numérico (COMEÇANDO DE UM, pois R é uma linguagem `r a_r("1-based")` ), é possível acessar os elementos pelos seus nomes
...
```{r}
vetor_nomeado["terceiro"]
```

Os nomes dos elementos podem ser acessados com a função `r a_r("names()")`
...
As `r a_r("listas nomeadas")` funcionam de forma similar aos vetores nomeados.
...
names(lista_nomeada) <- c("um", "dois", "tres")
...
```{r sem_pipe, message=FALSE, warning=FALSE }
...
![](imagens/dplyr_concept.png)

Fonte: [@dplyrrberlin]

## CRAN: bibliotecas de pacotes: funções e dados
...
```{r  candidatos,  message= FALSE, warning=FALSE, cache=TRUE }
...
estacao <- worldmet::getMeta("heathrow", returnMap = TRUE)
...
Outra forma mais nova de fazer isso é usando a função `r a_r("relocate()")`, que facilita a ordenação das colunas de um data frame
...
glimpse(dados_heathrow_relocate)
...
## Renomeando várias colunas de uma vez (dplyr): `rename_with()`

`r slide_atual = slide_atual + 1`

A função `r a_r("rename_with()")` possibilita que renomeemos várias colunas ao mesmo tempo com o uso de uma função.

Aqui fazemos uso de uma função da biblioteca `r a_r("stringr")`, que trata strings, ou seja, cadeias de caracteres.
...
A função `r a_r("mutate()")` é usada para criar novas colunas no tibble, mantendo as outras. ![](diagramas/dplyr_mutate.png)
...
candidatos_com_data <- candidatos %>% 
    mutate(DT_ELEICAO = dmy(DT_ELEICAO)) %>% 
    select(DT_ELEICAO)
...
```{r bets,  warning=FALSE, message=FALSE}
...
## Funções básicas de tratamento (dplyr) `slice_max()`:

`r slide_atual = slide_atual + 1`

`r a_r("slice_max()")` seleciona as `n` linhas maiores de acordo com uma das colunas.

![](diagramas/dplyr_slice_max.png)

## Funções básicas de tratamento (dplyr) `slice_max()` número de itens:
...
## Funções básicas de tratamento (dplyr) `slice_max()`, proporção das linhas:
...
gini_agrupado %<>% mutate(
    value_ant = lag(SI.POV.GINI),
    delta_value = SI.POV.GINI - value_ant
    )
...
A função `r a_r("slice_max() no contexto da group_by()")` retorna os n maiores valores. Se o tibble estiver agrupado, pra cada grupo.
...
## Operações column-wise

`r slide_atual = slide_atual + 1`

A biblioteca dplyr chegou à versão 1.0.0 em junho de 2020, estabelecendo sua interface permanente e ganhando novas funcionalidades, como as `r a_r("column-wise operations")`, com a função `r a_r("across()")`.
...
## Operações column-wise com mutate
...
-   O parâmetro `.fns` define a função (ou a lista de funções) que serão aplicadas em cada coluna
...
## Operações column-wise com filter
...
head(pisas_completos)


```

## Operações column-wise com summarise
...
## Operações column-wise múltiplas funções
...
## Olhando as list-columns com mais detalhes
...
No código seguinte, particionamos os dados do PISA por país e aninhamos em uma list-column usando a função `r a_r("nest()")`. Podemos dar group_by e depois nest ou explicitar as colunas que queremos aninhar.

É uma boa prática rodar `r a_r("ungroup")` assim que não for necessário usar o tibble agrupado
...
## Executando um modelo para cada um dos tibbles aninhados
...
pisa_modelo <- pisa_nested %>% 
  mutate(
    modelo = map(.x = pisa, .f = ~lm(P05 ~ date, data = .x) )
  )
  

```

## Operações row-wise. Exemplo: extraindo coeficientes de modelos aninhados
...
Portanto, essa operação deve ser feita linha a linha. Para que isso aconteça é necessário rodar a função `r a_r("rowwise")`, que equivale a um `group_by()` por linha.

```{r}

pisa_modelo_coefs <- pisa_modelo %>% 
  rowwise() %>% 
  mutate(
    coeficientes = list(broom::tidy(modelo))
  ) %>% 
  ungroup()
...
## Explodindo tibbles aninhados

`r slide_atual = slide_atual + 1`

A função `broom::tidy()` criou novos tibbles aninhados com as informações a respeito dos coeficientes estimados.

Vamos, agora explodir esses tibbles, com a função `r a_r("unnest()")`, mostrando o coeficiente que nos interessa aqui, o $\beta$ relativo à variável explicativa contendo o ano.
...
baixa_faltantes <- tibble(data_dado = seq(ymd("2019-01-01"), by = "month", ymd(today()-4) )) %>% 
    ...
    select(-data_formato) %>% 
    mutate(data = pmap(.l = list(endereco = endereco, arquivo = arquivo), salva))



```

## Leitura de dados de arquivos em uma pasta - Exemplo: CVM. Desaninhando uma list-column
...
Usamos `r a_r("execução em paralelo no exemplo")`, que é possibilitada ao chamarmos uma função future\_<map> ao invés de <map>.

Após a leitura dos arquivos eles ficam armazenados de forma aninhada numa coluna chamada "conteudo". "conteudo" é uma coluna feita de tibbles, cada tibble correspondendo a uma linha do tibble principal. A coluna "conteudo" é uma `r a_r("nested list-column")`.
...
A função `r a_r("unnest")` pode ser usada, como já vimos
...
Primeiro usamos a função `r a_r("crossing")` para criar um novo tibble que combina todas as $n$ linhas do tibble com os grupos e todas as $m$ linhas do tibble de datas, resultando num tibble com $m * n$ linhas, pois queremos pegar o resultado de todos grupos para todas as datas.

```{r ceagesp1, message=TRUE, warning=TRUE, cache=TRUE}
...
download.file(url = unlist(endereco), destfile = "dados/aneel/gd.zip")
...
unzip(zipfile = "dados/info_empresas_cvm/info.zip", exdir = "dados/info_empresas_cvm/unzip" )
...
`r slide_atual = slide_atual + 1` Use `r a_r("small multiples")`: divida o gráfico em gráficos iguais menores cada um com parte dos dados, seguindo uma regra categórica
...
-   o nome da coluna pra onde os valores devem ir (equivalente ao parâmetro `values_to`)
-   um valor de atributo que deve ir para uma nova coluna

Além disso, uma transformação é aplicada nos nomes das colunas de forma a possibilitar o tratamento no mutate posterior.
...
```{r warning=FALSE, message=FALSE, out.width="80%", fig.height= 6, fig.width= 6  }
...
```{r, message=FALSE, warning=FALSE, out.width="80%", fig.height= 6, fig.width= 6}
...
## Modelo de aprendizado estatistico: Fábrica de função que aproxima a realidade
...
![](diagramas/parametro_sem_parametro.png)
...
![](diagramas/erro_redutivel_irredutivel.png)
...
## Trade-off interpretabilidade x acurácia e viés x variância
...
#### Viés x variância e Ponto ótimo no aprendizado e overfitting
...
-   `r a_r("Dados de validação")`: servem para comparar modelos de forma que nos possibilite escolher UM modelo com UMA especificação de hiperparâmetros (número de nós da rede neural, sensibilidade da penalização pelo tamanho e número dos coeficientes nas regressões lasso e ridge etc. )
...
ufc_raw_fighter <- read_csv("dados/ufc/raw_fighter_details.csv") 

ufc_raw_pre <- read_csv("dados/ufc/preprocessed_data.csv") 
...
modelo_ufc_por_classe <- ufc_aprend %>% 
  ...
  nest() %>%
  mutate(
    modelo = map(.x = data, ~lm(zebra_blue ~ idade_red  + red_mais_velho, data = .x)),
  ) %>% 
  select(-data) %>% 
  view()
...
Plotando as regiões deste modelo mais complexo `r slide_atual = slide_atual + 1`
...
![](imagens/reactivity-graph-00.png){width="80%"}
...
![](imagens/reactlog1.png){width="80%"}

O caminho até o primeiro objeto válido é seguido, então, pra cada elemento inválido

![](imagens/reactlog2.png){width="80%"}

![](imagens/reactlog3.png){width="80%"}

![](imagens/reactlog4.png){width="80%"}
...
![](imagens/reactlog5.png){width="80%"}

Isso invalida todos os objetos que dependem dele

![](imagens/reactlog6.png){width="80%"}

O processo de cálculo é seguido de novo

![](imagens/reactlog7.png){width="80%"}
...
![](imagens/reactlog8.png){width="80%"}


No momento em que clicamos no botão, todos os elementos dependentes são invalidados


![](imagens/reactlog9.png){width="80%"}

![](imagens/reactlog10.png){width="80%"}
...
eventos <- read_feather(here("dados/football_events/gameevents.feather"))

dicionario <- read_lines(here("dados/football_events/dictionary.txt")) %>%
    enframe(
        value = "linha"
    ) %>%
    mutate(
        inicio_tabela = str_detect(linha, "^[a-z]"),
        vazio = str_length(linha) == 0,
        tabela = cumsum(inicio_tabela)
    ) %>%
    filter(
        !vazio
    ) %>%
    ...
    mutate(
        conteudo_limpo = map(
            .x = data, .f = ~janitor::row_to_names(dat = .x, row_number = 1, remove_row = TRUE)
        )
    ) %>%
    unnest(
        conteudo_limpo
...
eventos %>% 
    head(100) %>% 
    left_join(
        dicionario %>% filter(campo == "event_type") %>% rename(event_type_str = valor),
        by = c("event_type" = "cod")
    ) %>% 
    left_join(
        dicionario %>% filter(campo == "event_type2") %>% rename(event_type2_str = valor),
        by = c("event_type2" = "cod")
    ) %>%
    left_join(
        dicionario %>% filter(campo == "shot_place") %>% rename(shot_place_str = valor),
        by = c("shot_place" = "cod")
    ) %>%
    left_join(
        dicionario %>% filter(campo == "shot_outcome") %>% rename(shot_outcome_str = valor),
        by = c("shot_outcome" = "cod")
    ) %>%
    left_join(
        dicionario %>% filter(campo == "location") %>% rename(location_str = valor),
        by = c("location" = "cod")
    ) %>%
    left_join(
        dicionario %>% filter(campo == "bodypart") %>% rename(bodypart_str = valor),
        by = c("bodypart" = "cod")
    ) %>%
    left_join(
        dicionario %>% filter(campo == "assist_method") %>% rename(assist_method_str = valor),
        by = c("assist_method" = "cod")
    ) %>%
    left_join(
        dicionario %>% filter(campo == "situation") %>% rename(situation_str = valor),
        by = c("situation" = "cod")
    )

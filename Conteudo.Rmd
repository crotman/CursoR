...
gini = wb_data( indicator = "SI.POV.GINI", mrnev = 10)
...
salva <- function(endereco, arquivo){
    ...
    read_csv2(endereco) %>% 
      mutate(
        across(
          -c("CNPJ_FUNDO", "NR_COTST", "DT_COMPTC"),
          as.numeric
        )
      ) %>% 
      mutate_at(
...
A função `r a_r("unnest")` pode ser usada 


```{r cadastro_fundos,  warning=FALSE, message=FALSE}

download.file(url = "http://dados.cvm.gov.br/dados/FI/CAD/DADOS/cad_fi_hist.zip", destfile = "dados/cvm/cad_fi_hist.zip" )

unzip(zipfile = "dados/cvm/cad_fi_hist.zip", exdir =  "dados/cvm")


cadastro_fundos <- read_csv2("dados/cvm/cad_fie.csv", locale = locale(encoding = "latin1"))

maraus <- cadastro_fundos %>% 
  filter(
    str_detect(DENOM_SOCIAL, "MARA")
  )

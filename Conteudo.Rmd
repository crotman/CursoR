...
Existem fábricas para todos os gostos.

Esta fábrica de $\hat{f}()$ recebe como insumo variáveis retiradas do ambiente. Essas vraiáveis podem ser tratadas de forma a deixar as coisas mais fáceis para o modelo, de modo a deixá-lo na cara do gol como Gérson fazia na copa de 70. Esta etapa de preparar as entradas se chama Feature Engiineering. Para fazer isso, usamos tudo que aprendemos sobre manipulação de dados.
...
Com o(s) modelo(s) treinado(s), faremos inferências a respeito de outros dados futuros ou cuja variável dependente ainda desconhecemos.


## Seleção de modelo e Trade-off viés-variância

Nosso modelo pode ser resumido por uma equação:

$$y = f(x) + \epsilon$$


Onde, idealmente, $E(\epsilon) = 0$, $\epsilon$ é independente de $x$ e $Var(\epsilon) = \sigma^2$ é constante em relação a $x$. 


Ou seja, temos o objetivo de chegar à $f()$ mais próxima da ideal com a ajuda de nossos modelos, e existem vários tipos deles. 


Podemos chamar a nossa estimativa de função ideal de $\hat{f}(x)$


## Seleção de modelo e Trade-off viés-variância


Se usarmos o erro quadrático como ffunção de penalização, podemos escrever a expectativa de erro de uma previsão a respeito de $x_0$ assim:


## Tentativa de estimar variância e viés: Cross validation
...
Primeiro devemos fazer nossa exploração preliminar.

Podemos executar um pequeno tratamento de Features usando a função `pivot_longer()` de um jeito diferente, só possível na últiam versão
...
```{r varias_espec_purr, message=FALSE, warning=FALSE, cache=TRUE}  

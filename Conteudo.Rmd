...
Vamos usar uma das funções da família `r<familia de distribuição de prob>()`. Neste caso, a `r a_r("rbinom")`, que simula a distribução binomial (aquela que equivale ao evento de jogar n moedas (ou alguma coisa com dois lados) para cima e ver quantas deram cara).
...
Na função `rbinom()`, a cada simulação, sorteamos uma de duas categorias para cada um dos n eventos. A `r a_r("rmultinom()")` sorteia uma de k categorias para cada um dos n eventos. Um vetor de k elementos indica a probabilidade de cada categoria.
...
`r a_r("dplyr")` é a biblioteca mais básica do conjunto `tidyverse`
...
De modo geral, as funções da biblioteca `r a_r("readr")` são mais rápidas do que as da biblioteca base, e também mostram barra de progresso no console. É possível reconhecê-las pelo `_` ao invés de `.` 
...
Note que vamos usar um `r a_r("exemplo de programação funcional")`. Semm uso de loop vamos executar uma função para cada linha do data frame/tibble.
...
Aqui fazemos o primeiro uso de uma funçãoda biblioteca `r a_r("stringr")`, que trata strings, ou seja, cadeias de caracteres. 
...
Veja como as funções da biblioteca `r a_r("kable")` e `r a_r("kableExtra")` dão mais controle na criação das tabelas.
